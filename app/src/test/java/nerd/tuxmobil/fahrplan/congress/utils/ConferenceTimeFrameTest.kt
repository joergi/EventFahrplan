...
class ConferenceTimeFrameTest {
    ...
    @Test
    fun `firstDayStart returns the the first day`() {
    ...
    @Test
    fun `contains returns false if time marks a session at the last day`() {
        assertThat(conference.contains(LAST_DAY_END_TIME)).isFalse()
    }

    @Test
    fun `endsBefore returns true if time marks a session at the last day`() {
        assertThat(conference.endsBefore(LAST_DAY_END_TIME)).isTrue()
    }

    @Test
    fun `endsBefore returns false if time marks a session starting one second before the last day`() {
